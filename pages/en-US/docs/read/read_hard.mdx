...
## Guide to Complex Reading Excel Files
### Overview

This guide explains how to use FastExcel to accomplish complex reading of Excel files in various scenarios. The scenarios covered include reading by column name or index, reading multiple sheets, converting date and number formats, parsing multi-row headers, reading additional information (such as comments, hyperlinks, merged cells), handling formulas, and exceptions.
...
### **Date, Number, Custom Format Conversion**
...
### Overview
By setting the `headRowNumber` parameter or automatically parsing multi-row headers based on annotations in the entity class.
...
#### Overview
Read Excel data synchronously and directly return a list of data. Suitable for small data scenarios, not recommended for large data volumes.
...
### **Reading Extra Information (Comments, Hyperlinks, Merged Cells)**

#### Overview
Read extra information, such as comments, hyperlinks, and merged cells, by setting the `extraRead` parameter.
...
public class DemoExceptionListener extends AnalysisEventListener<ExceptionDemoData> {
    ...
    public void onException(Exception exception, AnalysisContext context) {
        ...
        if (exception instanceof ExcelDataConvertException) {
            ...
            log.error("Exception parsing row {}, column {}, data: {}", ex.getRowIndex(), ex.getColumnIndex(), ex.getCellData());
        }
    }
