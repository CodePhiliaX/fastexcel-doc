...
## Overview
The complex writing feature satisfies diverse Excel data export requirements by supporting features such as specifying column order, dynamic column headers, merging cells, and custom styles.
...
### Overview
Specify the column order using the `index` property of the `@ExcelProperty` annotation.
...
### Overview
Write data to the same sheet or multiple sheets in batches, enabling pagination writing for large datasets.
...
### Overview
Supports date, number, or other custom formats using annotations.
...
If existing strategies cannot meet requirements, implement the `CellWriteHandler` interface to fully customize style control.
...
Directly manipulate POI's `CellStyle` for precise style control.
...
| Complex Condition Control of Styles | Custom Interceptor | Example 2-2       |
...
### Overview
Supports exporting images using various methods such as files, streams, byte arrays, URLs, etc.
...
### Overview
Generate table headers dynamically for scenarios with dynamic data or internationalization needs.
...
### Overview
Add comments to specific cells using interceptors, suitable for marking explanations or special notes.
...
### Overview
Supports filling data in an existing template file, suitable for standardized output.
...
### Overview
Control column width and row height using annotations, suitable for scenarios with specific table format requirements.
...
## **Using Table for Writing**

### Overview
Supports writing multiple tables in a single sheet.
...
### Overview
Generate dynamic table headers for scenarios where header content dynamically changes.
...
### Overview
Custom logic implementation (e.g., adding dropdowns, setting hyperlinks) requires operation through interceptors.
